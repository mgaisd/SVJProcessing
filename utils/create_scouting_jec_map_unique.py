#!/usr/bin/env python3
"""
Create Jet Energy Correction maps for scouting jets (UNIQUE MATCHING VERSION).
...
- Implements unique matching: each offline jet matched to at most one scout jet
- Uses greedy minimum deltaR matching with conflict resolution
- Slightly slower (~2x) but ensures no double-counting

Performance optimizations:
- Vectorized jet matching using awkward arrays
- Single-pass file processing for both Jet and FatJet (2x I/O speedup)
- Numpy broadcasting for efficient ΔR calculations
- Vectorized conflict resolution using lexsort
...
def match_jets_by_dr_vectorized(scout_pt, scout_eta, scout_phi, 
    ...
    """
    Match scouting jets to offline jets using explicit event loop with unique matching.
    Each offline jet can only be matched to ONE scout jet (the closest one).
    ...
    # Loop over each event explicitly
    n_events = len(scout_pt)
    for i_evt in range(n_events):
        # Get jets for this event (convert to numpy)
        s_pt = ak.to_numpy(scout_pt[i_evt])
        s_eta = ak.to_numpy(scout_eta[i_evt])
        s_phi = ak.to_numpy(scout_phi[i_evt])
        o_pt = ak.to_numpy(offline_pt[i_evt])
        o_eta = ak.to_numpy(offline_eta[i_evt])
        o_phi = ak.to_numpy(offline_phi[i_evt])
        
        n_scout = len(s_pt)
        n_offline = len(o_pt)
        
        n_scout_total += n_scout
        
        if n_scout == 0 or n_offline == 0:
            continue
        
        # Store matches: (scout_idx, offline_idx, deltaR)
        matches = []
        
        # For each scout jet, find best matching offline jet
        for i_scout in range(n_scout):
            best_dr = dr_threshold
            best_offline_idx = -1
            
            for i_offline in range(n_offline):
                # Compute deltaR
                deta = s_eta[i_scout] - o_eta[i_offline]
                dphi = s_phi[i_scout] - o_phi[i_offline]
                # Wrap phi to [-pi, pi]
            ...
            if best_offline_idx >= 0:
                matches.append((i_scout, best_offline_idx, best_dr))
        
        if len(matches) == 0:
            continue
        
        # Resolve conflicts: if multiple scout jets match the same offline jet,
        # keep only the closest match
        matches_array = np.array(matches, dtype=[('scout', 'i4'), ('offline', 'i4'), ('dr', 'f4')])
        
        # Sort by offline index, then by deltaR
        matches_array.sort(order=['offline', 'dr'])
        
        # Keep first occurrence of each unique offline jet (smallest deltaR)
        unique_offline = []
        final_matches = []
        for match in matches_array:
            if match['offline'] not in unique_offline:
                unique_offline.append(match['offline'])
                final_matches.append(match)
        
        # Extract matched pT values
        for match in final_matches:
            i_scout = match['scout']
            i_offline = match['offline']
            
            # Sanity checks
            if s_pt[i_scout] <= 0 or o_pt[i_offline] <= 0:
                continue
            if not np.isfinite(s_pt[i_scout]) or not np.isfinite(o_pt[i_offline]):
                continue
            
            matched_scout_pt.append(s_pt[i_scout])
            matched_scout_eta.append(s_eta[i_scout])
            matched_offline_pt.append(o_pt[i_offline])
            n_matched += 1
    
    # Convert to numpy arrays
    if len(matched_scout_pt) > 0:
        final_scout_pt = np.array(matched_scout_pt)
        final_scout_eta = np.array(matched_scout_eta)
        final_offline_pt = np.array(matched_offline_pt)
    else:
        final_scout_pt = np.array([])
        final_scout_eta = np.array([])
        final_offline_pt = np.array([])
